...
def encrypt_text(algorithm, key, block_size, plaintext,iv):
	if algorithm == "Twofish":
		ciphertext = iv+encrypt_twofish(plaintext.encode(), key, iv)
		return ciphertext
	else:
...
def encrypt_file(algorithm, key,iv, block_size, input_file, output_file, hex_output_file):
	with open(os.getcwd() + '/temp/' + input_file, "rb") as f:
		plaintext = f.read()
	if algorithm == "Twofish":
		ciphertext = iv+encrypt_twofish(plaintext, key, iv)
...
def decrypt_file(algorithm, key, block_size, input_file, output_file):
	with open(os.getcwd() + '/temp/' + input_file, "rb") as f:
		file_data = f.read()
	...
	if algorithm == "Twofish":
		...
		ciphertext = file_data[block_size:]
		cipher = algorithm.new(key, algorithm.MODE_CBC, iv)
		decrypted_data = cipher.decrypt(ciphertext)  # Decrypt only data
...
def decrypt_and_save(algorithm, key, block_size, input_file):
    ...
    decrypt_file(algorithm, key, block_size, input_file, decrypted_hex_file)
...
def decrypt_simple(algorithm, key, block_size, input_file):
    ...
    if is_hex_file(input_file):
        ...
        return decrypt_and_save(algorithm, key, block_size, temp_binary_file)
    
    return decrypt_and_save(algorithm, key,block_size, input_file)
